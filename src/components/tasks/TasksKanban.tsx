...
import { Calendar, GripVertical, AlertTriangle, ListChecks, FolderKanban, ArrowLeft } from 'lucide-react';
...
export function TasksKanban({ tasks, onTaskClick, onStatusChange }: TasksKanbanProps) {
  ...
  // ─── Focus mode ─────────────
  if (focusedColumn) {
    const column = columns.find(c => c.id === focusedColumn);
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div className={cn('w-3 h-0.5 rounded', column.color.replace('border-', 'bg-'))} />
          ...
          {columnTasks.map((task) => {
            ...
            return (
              <Card
                key={task.id}
                className={cn(
                  'cursor-pointer transition-all duration-200 group border-border/60 hover:shadow-md hover:border-border',
                  isDone && 'opacity-60',
                )}
                    ...
                    {(task.task_contacts?.[0]?.contacts?.full_name || task.task_contacts?.[0]?.contacts?.company) && (
                      <span className="text-muted-foreground font-normal ml-1">
                        ...
                        {task.task_contacts[0].contacts.company && (
                          <span className="text-muted-foreground/70 ml-1">· {task.task_contacts[0].contacts.company}</span>
                        )}
                      </span>
                    )}
                  </h4>
        ...
        <KanbanInlineCreate status={focusedColumn} />
      </div>
    );
  }

  // ─── Normal Kanban with carousel hover ─────────────
  return (
    <div
    ...
    >
      {columns.map((column) => {
        ...
        const isShrunk = !!hoveredColumn && hoveredColumn !== column.id;
        ...
        return (
          <div
            ...
            className={cn(
              ...
              isHovered ? 'flex-[3]' : isShrunk ? 'flex-[0.7]' : 'flex-1'
            )}
          ...
          >
            ...
            >
              {/* Column header */}
              <div
                className="flex items-center justify-between px-3 py-2.5 border-b border-border/50 cursor-pointer hover:bg-muted/50 transition-colors"
