...
import { useDealTeamContactByPk } from '@/hooks/useOdprawaAgenda';
...
export function StalledContactsCard() {
  ...
  const [selectedDtcId, setSelectedDtcId] = useState<string | null>(null);
  const sheetContactQ = useDealTeamContactByPk(selectedDtcId);

  useEffect(() => {
    if (sheetContactQ.error) {
      toast.error('Nie udało się wczytać kontaktu');
      setSelectedDtcId(null);
    }
  }, [sheetContactQ.error]);
                      ...
                      <TableRow key={c.id}>
                        ...
                        <TableCell>
                          <Button
                            ...
                            size="sm"
                            className="h-7"
                            onClick={() => setSelectedDtcId(c.id)}
                            disabled={sheetContactQ.isFetching && selectedDtcId === c.id}
                          >
