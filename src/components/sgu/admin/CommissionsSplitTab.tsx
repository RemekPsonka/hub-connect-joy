...
// TODO: dodanie inline edit udziałów (ProductCategory × rate_*) w SGU-08
interface SplitRow {
  id: string;
  role_key: string;
...
export function CommissionsSplitTab() {
  ...
  const sumActive = useMemo(() => activeSplits.reduce((s, r) => s + Number(r.share_pct), 0), [activeSplits]);
  const sumWarning = Math.abs(sumActive - 100) > 0.01;

  return (
    <div className="space-y-4">
      <Card>
        <CardHeader><CardTitle>Konfiguracja prowizji — split bazowy (Case A)</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Aktywny split definiuje udziały bazowe. Przy obecności <code>has_handling</code> lub
            <code> representative_user_id</code> trigger stosuje warianty B/C/D.
          </p>
          ...
          {sumWarning && (
            <Alert variant="destructive">
              ...
              <AlertDescription>Aktualna suma: {sumActive.toFixed(2)}%. Skoryguj rekordy.</AlertDescription>
            </Alert>
          )}
          ...
          {isLoading ? (
            ...
            <Alert variant="destructive">
              ...
              <AlertDescription>Trigger zwróci błąd przy próbie generowania prowizji.</AlertDescription>
            </Alert>
          ) : (
              ...
              <Table>
                ...
                <TableBody>
                  {activeSplits.map((row) => (
                    <TableRow key={row.id}>
                      <TableCell className="font-medium">{ROLE_LABELS[row.role_key] ?? row.role_key}</TableCell>
                      <TableCell className="text-xs text-muted-foreground">
                        {row.recipient_user_id ? row.recipient_user_id.slice(0, 8) + '…' : '—'}
                      </TableCell>
                      <TableCell className="text-right font-semibold tabular-nums">{row.share_pct}%</TableCell>
                      <TableCell className="text-xs">{new Date(row.active_from).toLocaleDateString('pl-PL')}</TableCell>
          ...
          {!canEdit && (
            <p className="text-xs text-muted-foreground italic">
              Edycja dostępna dla partnera SGU lub dyrektora.
            </p>
          )}
        </CardContent>
      </Card>
      ...
      {archivedSplits.length > 0 && (
        <Card>
          <CardHeader><CardTitle className="text-base">Historia wersji</CardTitle></CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow>
                ...
                {archivedSplits.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell className="text-xs text-muted-foreground">
                      ...
                      {row.active_to ? new Date(row.active_to).toLocaleDateString('pl-PL') : 'aktywny'}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
