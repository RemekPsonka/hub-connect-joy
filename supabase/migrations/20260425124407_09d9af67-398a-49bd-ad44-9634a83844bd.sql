...
BEGIN
  ...
  ELSIF NEW.decision_type = 'dead' THEN
    UPDATE public.deal_team_contacts
       SET is_lost            = true,
           lost_reason        = NEW.dead_reason,  -- FIX #27: kolumna na meeting_decisions to dead_reason
           lost_at            = now(),
           category           = 'lost',
           deal_stage         = 'lost',
           status             = 'disqualified',
...
-- ROLLBACK:
-- CREATE OR REPLACE FUNCTION public.apply_meeting_decision() ... (wersja z NEW.lost_reason w branchu 'dead')