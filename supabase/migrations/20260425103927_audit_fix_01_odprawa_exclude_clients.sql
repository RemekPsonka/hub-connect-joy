-- AUDIT-FIX-01: get_odprawa_agenda wyklucza klientów (category='client')
-- Odprawa = lejek sprzedaży. Klienci mają własny moduł (/sgu/klienci).
-- ROLLBACK: usuń linię "AND d.category <> 'client'" z WHERE w CTE dtc.
...
BEGIN
  ...
  WITH dtc AS (
    ...
    WHERE d.team_id = p_team_id
      ...
      AND d.category <> 'client'  -- AUDIT-FIX-01: klienci w osobnym module
  ),
  task_counts AS (
    SELECT
