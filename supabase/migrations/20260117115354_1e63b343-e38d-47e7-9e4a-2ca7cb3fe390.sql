...
DROP FUNCTION IF EXISTS search_all_hybrid(text, vector, uuid, text[], double precision, double precision, double precision, integer);

-- Drop and recreate the function with clear signature
DROP FUNCTION IF EXISTS search_all_hybrid(text, uuid, text[], text, double precision, integer, double precision, double precision);
...
BEGIN
  ...
  SELECT array_agg(term) INTO v_expanded_terms
