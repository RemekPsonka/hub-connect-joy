-- Fix get_odprawa_agenda RPC (Wariant C+):
-- 1) deal_team_contacts.stage -> category (alias as stage to keep FE contract)
-- 2) 'ofertowanie' -> 'offering'
-- 3) next_action_date::timestamptz cast (column is date, RETURNS declares timestamptz)
-- 4) bucket '10x' = (temperature='10x' OR category='10x')
-- ROLLBACK: previous body referenced d.stage, 'ofertowanie', no cast, only temperature='10x'.
