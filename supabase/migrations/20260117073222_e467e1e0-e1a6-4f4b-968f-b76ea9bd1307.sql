-- ============================================
-- FIX: Handle multi-word synonyms in search_all_hybrid
-- Problem: Synonyms like "umowa ubezpieczeniowa" cause tsquery syntax errors
-- Solution: Filter out multi-word terms OR convert spaces to phrase search
-- ============================================
...
BEGIN
  ...
  SELECT array_agg(DISTINCT term) INTO v_expanded_terms
  FROM expand_search_query(p_query);
  ...
  IF v_expanded_terms IS NOT NULL AND array_length(v_expanded_terms, 1) > 0 THEN
    ...
    WHERE term IS NOT NULL 
      AND trim(term) != ''
      AND term !~ '\s';  -- Skip terms containing spaces
  ELSE
    -- Fallback to original query if no valid terms
...
-- Also clean up problematic multi-word synonyms from the table
...
WHERE EXISTS (
  SELECT 1 FROM unnest(synonyms) AS s WHERE s ~ '\s'
);