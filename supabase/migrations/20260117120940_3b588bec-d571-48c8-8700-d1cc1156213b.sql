...
BEGIN
  ...
  SELECT string_agg(t || ':*', ' | ')::tsquery INTO v_fts_query
  ...
  WITH fts_results AS (
    SELECT 
      ...
      CASE WHEN lower(c.full_name) LIKE '%' || v_query_lower || '%' THEN 0.3 ELSE 0 END::double precision as item_title_boost
    FROM contacts c
    ...
    SELECT n.id as item_id, 'need'::text as item_type, n.title as item_title,
      ...
      LEAST(ts_rank(n.fts, v_fts_query) * 10, 1.0)::double precision as item_fts_score,
      0::double precision as item_semantic_score,
      CASE WHEN lower(n.title) LIKE '%' || v_query_lower || '%' THEN 0.3 ELSE 0 END::double precision as item_title_boost
    FROM needs n
    ...
    SELECT o.id as item_id, 'offer'::text as item_type, o.title as item_title,
      ...
      LEAST(ts_rank(o.fts, v_fts_query) * 10, 1.0)::double precision as item_fts_score,
      0::double precision as item_semantic_score,
      CASE WHEN lower(o.title) LIKE '%' || v_query_lower || '%' THEN 0.3 ELSE 0 END::double precision as item_title_boost
    FROM offers o
