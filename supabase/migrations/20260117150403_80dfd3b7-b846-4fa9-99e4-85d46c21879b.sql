-- FIX 4a: Update search_all_hybrid to return contact_id for proper navigation
DROP FUNCTION IF EXISTS public.search_all_hybrid(text, text, uuid, text[], double precision, double precision, double precision, integer);
...
RETURNS TABLE(
  ...
  subtitle text, 
  description text, 
  contact_id uuid,  -- NEW: Added contact_id for navigation
...
BEGIN
  ...
  WITH fts_results AS (
    ...
    SELECT 
      ...
      c.id as item_contact_id,  -- For contacts, contact_id is self
    ...
    -- Needs: include contact_id for navigation
    SELECT 
      n.id as item_id, 
      ...
      n.contact_id as item_contact_id,  -- Include contact_id
    ...
    -- Offers: include contact_id for navigation
    SELECT 
      o.id as item_id, 
      ...
      o.contact_id as item_contact_id,  -- Include contact_id
  ...
  semantic_results AS (
    ...
    -- Needs: semantic search with contact_id
    SELECT 
      n.id as item_id, 
      'need'::text as item_type, 
      n.title as item_title,
    ...
    -- Offers: semantic search with contact_id
    SELECT 
      o.id as item_id, 
      'offer'::text as item_type, 
      o.title as item_title,
  ...
  combined AS (
    SELECT 
      ...
      COALESCE(f.item_contact_id, s.item_contact_id) as item_contact_id,  -- Combine contact_id
  ...
  SELECT 
    ...
    cb.item_subtitle, 
    cb.item_description,
    cb.item_contact_id,  -- Return contact_id
    cb.item_fts_score, 
