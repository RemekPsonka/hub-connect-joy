-- Improved search_all_hybrid with normalized ts_rank, title boost, and higher semantic threshold
...
BEGIN
  ...
  -- Build FTS query (skip multi-word terms)
  ...
  RETURN QUERY
  WITH fts_results AS (
    -- Contacts FTS with normalized score + title boost
    SELECT 
      c.id as item_id,
      'contact'::text as item_type,
      c.full_name as item_title,
      COALESCE(c.position, '') || 
        CASE WHEN c.company IS NOT NULL THEN ' @ ' || c.company ELSE '' END as item_subtitle,
      c.profile_summary as item_description,
      LEAST(ts_rank(c.fts, v_fts_query) * 10, 1.0)::double precision as item_fts_score,
      0::double precision as item_semantic_score,
      CASE WHEN lower(c.full_name) LIKE '%' || v_query_lower || '%' 
           THEN 0.3 ELSE 0 END::double precision as item_title_boost
    FROM contacts c
    ...
    -- Needs FTS
    SELECT 
      n.id, 'need'::text, n.title,
      (SELECT ct.full_name FROM contacts ct WHERE ct.id = n.contact_id),
      n.description,
      LEAST(ts_rank(n.fts, v_fts_query) * 10, 1.0)::double precision,
      0::double precision,
      CASE WHEN lower(n.title) LIKE '%' || v_query_lower || '%' 
           THEN 0.3 ELSE 0 END::double precision
    FROM needs n
    ...
    -- Offers FTS
    SELECT 
      o.id, 'offer'::text, o.title,
      (SELECT ct.full_name FROM contacts ct WHERE ct.id = o.contact_id),
      o.description,
      LEAST(ts_rank(o.fts, v_fts_query) * 10, 1.0)::double precision,
      0::double precision,
      CASE WHEN lower(o.title) LIKE '%' || v_query_lower || '%' 
           THEN 0.3 ELSE 0 END::double precision
    FROM offers o
  ...
  semantic_results AS (
    ...
    SELECT 
      c.id, 'contact'::text, c.full_name,
      COALESCE(c.position, '') || 
        CASE WHEN c.company IS NOT NULL THEN ' @ ' || c.company ELSE '' END,
      c.profile_summary,
      0::double precision,
      (1 - (c.profile_embedding <=> v_embedding))::double precision,
      0::double precision
    FROM contacts c
    ...
    SELECT 
      ...
      n.description,
      0::double precision,
      (1 - (n.embedding <=> v_embedding))::double precision,
      0::double precision
    FROM needs n
    WHERE v_use_semantic
    ...
    SELECT 
      ...
      o.description,
      0::double precision,
      (1 - (o.embedding <=> v_embedding))::double precision,
      0::double precision
    FROM offers o
    WHERE v_use_semantic
  ...
  SELECT 
    ...
    LEAST(
      (cb.item_fts_score * p_fts_weight + 
       cb.item_semantic_score * p_semantic_weight + 
       cb.item_title_boost), 
      1.0
    )::double precision,
    CASE 
  ...
  ORDER BY (cb.item_fts_score * p_fts_weight + 
            cb.item_semantic_score * p_semantic_weight + 
            cb.item_title_boost) DESC
  LIMIT p_limit;
END;
$$ LANGUAGE plpgsql;