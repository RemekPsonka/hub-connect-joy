-- Fix ambiguous column reference in search_all_fts function
-- Use table alias 'contacts_lookup' in subqueries to avoid conflict with function's return column 'id'
...
BEGIN
  ...
  -- Needs with fixed subquery alias
  SELECT 
    n.id,
    'need'::text,
    n.title,
  ...
  -- Offers with fixed subquery alias
  SELECT 
    o.id,
    'offer'::text,
    o.title,
